...
POSSIBLE_PROBS = [0.1, 0.3, 0.7, 0.9]
BETWEEN_TRIALS_TIME = 500
...
class SimpleDecisionTask(object):
    def __init__(self, num_of_repetitions, manual_game=True, my_agent=None):
        ...
        self.round_scores_titles = ["Round Index", "Reaction Time", "left color", "left reward in current trial",
                                    ...
                                    "Clicked Key", "Reward"]
        if manual_game:
            self.between_trials_time = BETWEEN_TRIALS_TIME
        else:
            self.between_trials_time = 0 # int(BETWEEN_TRIALS_TIME / 100)   the computer does not need 500ms

        self.possible_probs = POSSIBLE_PROBS
        ...
        self.num_of_rounds = len(self.rounds)  # don't need to bring it back to num of repititions
