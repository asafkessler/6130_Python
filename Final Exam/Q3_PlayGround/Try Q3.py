...
# sorting the data according to the max change
...
if show_graphs == True:
    ...
    df_for_graph = df_for_graph.reset_index(names='cronological order')
    df_extreme_points = final_df.merge(df_for_graph, how="inner",
                                       on=["Year", "Month", "Day", "percent change", "one_USD_in_ILS"])

    ax = df_for_graph.plot(x='cronological order', y='one_USD_in_ILS', label= 'ILS for USD', linestyle='-', alpha = 0.5 )

    df_extreme_points.plot.scatter(x='cronological order', y='one_USD_in_ILS',label='extreme days', ax=ax,linestyle='', c = 'r', alpha=1)
    plt.pyplot.show()
